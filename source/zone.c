...
void Memory_Display_f(void)
{
	...
	{
		if (strcasecmp(Cmd_Argv(counter),"short") == 0) all = false;
		else if (strcasecmp(Cmd_Argv(counter),"save") == 0) write_file = true;
	}
...
void Cache_Display_f(void)
{
	...
	for(counter=1;counter<NumItems;counter++)
	{
		if (strcasecmp(Cmd_Argv(counter),"save") == 0) write_file = true;
	}
...
void Memory_Stats_f(void)
{
	...
	//
		...
		{
			if (strcasecmp(h->name,MemoryGroups[counter]) == 0)
			{
				GroupCount[counter]++;
